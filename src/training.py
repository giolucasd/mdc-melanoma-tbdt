...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        # logits: [batch, ...]
        # targets: [batch, ...] mesmo shape que logits

        # 1. Calcula o BCE com logits (mais estável numericamente)
        bce = F.binary_cross_entropy_with_logits(logits, targets, reduction="none")

        # 2. Calcula as probabilidades (necessário para o termo focal)
        p = torch.sigmoid(logits)
        
        # 3. Calcula pt (probabilidade da classe verdadeira)
        # Se target=1, pt=p. Se target=0, pt=1-p
        pt = p * targets + (1 - p) * (1 - targets)

        # 4. Calcula o termo de foco (Focal Term)
        focal_term = (1 - pt) ** self.gamma

        # 5. Calcula o termo Alpha
        # Se target=1, alpha_t = alpha. Se target=0, alpha_t = 1 - alpha
...
class MelanomaLitModuleGradCAM(pl.LightningModule):
    """
    Model agnostic LightningModule for melanoma classification.
    ...
    # Esse serve para análise com GradCAM
    def predict_step(self, batch, batch_idx):
        images, labels = batch
        logits = self(images).squeeze(1)
        return {"images": images, "logits": logits, "labels": labels}
